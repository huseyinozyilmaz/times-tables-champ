<template>
  <div class=" w-3/5 m-auto mt-4">
    ...
    <section class="font-black-ops-one">
      <div v-for="(record, index) in scores" class="flex gap-3 p-3 border m-3 rounded-lg border-gray-800 shadow-lg">
...
const { data: scores } = await useFetch(`${URL}?v=${new Date().valueOf() }`)
